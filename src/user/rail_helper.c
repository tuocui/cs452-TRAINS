...
int safe_distance_to_branch( train_t *train ) {
  int vel = (train->speeds[train->cur_speed]).str_vel;
...
int get_mm_past_last_landmark( train_t *train, int cur_time ) {
  ...
  int cur_vel = train->speeds[train->cur_speed].str_vel;
  int prev_vel = train->speeds[train->prev_speed].str_vel;
...
int time_to_node( train_t *train, int dist_to_node, track_node_t *track ) {
  ...
  if( speed_finish_time < cur_time ) {
    return time_to_dist_constant_vel( dist_to_node, train->speeds[train->cur_speed].str_vel );
  } else { // still changing velocity? fuck TODO: Figure this shit out
    int cur_velocity = ( (train->speeds[train->cur_speed].str_vel + 
                          train->speeds[train->prev_speed].str_vel) * 100 ) / 
                          ( ( speed_finish_time * 100 ) / cur_time );
    int dist_to_const_vel = ( ( speed_finish_time - cur_time ) * cur_velocity ) / 10000 + 
                            ( ( speed_finish_time - cur_time ) * ( train->speeds[train->cur_speed].str_vel - cur_velocity ) ) / 20000;
    // Will finish acceleration after we hit the node
    if( dist_to_const_vel > dist_to_node ) {
      return ( 200000 * dist_to_node ) / (cur_velocity + train->speeds[train->cur_speed].str_vel );
    } else { // will finish acceleration before we hit the node
      return speed_finish_time + time_to_dist_constant_vel( dist_to_node - dist_to_const_vel, train->speeds[train->cur_speed].str_vel );
    }
  }
}
